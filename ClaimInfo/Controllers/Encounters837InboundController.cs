...
{
    public class Encounters837InboundController : Controller
    {
        // GET: Encounters837Inbound
        ...
        private List<FileClaimDetails> GetFileDataForDashbaord()
        {
            ...
            if (!string.IsNullOrEmpty(sSelectedDate))
            {
                ...
                else if (sMenu == "Paid")
                {
                    ...
                    data = _entityContext.IntakeClaimDatas.Where(o => o.ClaimStatus == "Verified" && o.adjudication_status == adjudication_status && o.FileDate.Contains(sSelectedDate)).OrderByDescending(p => p.FileDate).Select(o => o.FileID).Distinct().ToList();
                    strClaimStatus = "Accepted";
                }
            ...
            using (HiPaaS_website_offshoreEntities entites = new HiPaaS_website_offshoreEntities())
            {
                ...
                if (!string.IsNullOrEmpty(sSelectedDate))
                {
                    ...
                    if (strClaimStatus != "")
                    {
                        foreach (var FileID in data)
                        {
                            if (strClaimStatus == "Verified" && adjudication_status != "")
                            {
                                foreach (FileInTake fileInTake in _entityContext.FileInTakes.Where(x => x.FileID == FileID))
                                {
                                    model.Add(new FileClaimDetails
